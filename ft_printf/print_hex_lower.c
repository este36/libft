...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/10/11 16:01:56 by emercier          #+#    #+#             */
/*   Updated: 2025/10/24 16:57:14 by emercier         ###   ####lausanne.ch   */
/*                                                                            */
/* ************************************************************************** */
...
static bool	get_val_hex_lower(t_ft_printf_spec *spec)
{
	char	*hex_str;

	hex_str = get_hex(*(unsigned int *)spec->user_data, HEX_LOWERCASE);
	if (!hex_str)
		return (false);
...
static bool	apply_precision_hex_lower(t_ft_printf_spec *spec)
{
	if (!get_val_hex_lower(spec))
		return (false);
...
int	print_hex_lower(va_list *arg_list, t_ft_printf_spec *spec)
{
	...
	spec->apply_precision = apply_precision_hex_lower;
